...
function getSavedViewMode(userEmail?: string | null): 'cards' | 'list' {
  if (typeof window === 'undefined') return 'cards';

  if (userEmail) {
    const clean = userEmail.trim().toLowerCase();
    const userKey = `cm_ideas_view_mode_${clean}`;
    const savedUser = localStorage.getItem(userKey) || getCookie(userKey);
    if (savedUser === 'cards' || savedUser === 'list') {
      return savedUser;
    }
  }

  const savedGeneral = localStorage.getItem('cm_ideas_view_mode') || getCookie('cm_ideas_view_mode');
  if (savedGeneral === 'cards' || savedGeneral === 'list') {
    return savedGeneral;
  }
...
export default function HomePage() {
  ...
  // Carrega a preferência de visualização do usuário ao iniciar ou quando o usuário logar/mudar
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const mode = getSavedViewMode(user?.email);
      setViewMode(mode);
    }
  }, [user?.email]);
  ...
  const handleSetViewMode = (mode: 'cards' | 'list') => {
    setViewMode(mode);
    if (typeof window !== 'undefined') {
      localStorage.setItem('cm_ideas_view_mode', mode);
      const maxAge = 60 * 60 * 24 * 365; // 1 ano
      document.cookie = `cm_ideas_view_mode=${mode}; path=/; max-age=${maxAge}; SameSite=Lax`;

      if (user?.email) {
        const clean = user.email.trim().toLowerCase();
        localStorage.setItem(`cm_ideas_view_mode_${clean}`, mode);
        document.cookie = `cm_ideas_view_mode_${encodeURIComponent(clean)}=${mode}; path=/; max-age=${maxAge}; SameSite=Lax`;
      }
    }
  };
  ...
  const [newProduct, setNewProduct] = useState<'Varejofacil'>('Varejofacil');
  const [newCategory, setNewCategory] = useState<string>('Estoque');
  ...
  const handleExecuteLogin = async (email: string, pass: string) => {
    ...
    try {
      ...
      // Restaura a preferência de visualização (Cards ou Lista) do usuário que está logando
      const userSavedMode = getSavedViewMode(cleanEmail);
      setViewMode(userSavedMode);
          ...
          <div className="max-w-md w-full bg-white/90 backdrop-blur-md border border-slate-200/80 rounded-3xl shadow-xl shadow-emerald-950/5 p-8 relative overflow-hidden">
            <div className="text-center mb-7 relative z-10">
              ...
              <p className="text-xs sm:text-sm text-slate-500 mt-1.5 leading-relaxed max-w-xs mx-auto">
                Sua voz constrói o futuro do <span className="text-emerald-700 font-semibold">Varejofacil</span>
              </p>
            </div>
            {loginStep === 'email' && (
              <form
    ...
    <div className="flex-1 flex flex-col min-h-screen bg-slate-50/70 text-slate-800 selection:bg-emerald-100 selection:text-emerald-900 font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
        ...
        {activeTab === 'board' && (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-teal-700 rounded-3xl p-6 sm:p-8 text-white relative overflow-hidden shadow-lg shadow-emerald-900/10">
              <div className="max-w-2xl relative z-10">
                ...
                <p className="text-emerald-100 text-sm mt-2 leading-relaxed">
                  Não sofra mais com planilhas paralelas e rotinas manuais. Compartilhe como você contorna os problemas no dia a dia. As melhorias mais votadas entram diretamente no planejamento do <strong>Varejofacil</strong>.
                </p>
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs space-y-3">
              <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
                  ...
                  {(['TODOS', 'Varejofacil'] as const).map((prod) => (
                    <button
                      key={prod}
                ...
                {/* Alternador de Visualização: Cards ou Lista (conforme card_lista.png) */}
                <div className="flex items-center bg-slate-100 border border-slate-200 rounded-xl p-1 shrink-0">
                  <button
                    type="button"
                    onClick={() => handleSetViewMode('cards')}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all duration-200 cursor-pointer outline-none focus:outline-none focus:ring-0 focus-visible:outline-none select-none ${
                      viewMode === 'cards'
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 animate-fade-in">
                {filteredIdeas.map((idea) => {
                  ...
                  return (
                    <div
                      key={idea.id}
                      className="bg-white border border-slate-200/80 hover:border-emerald-300 rounded-3xl p-5 sm:p-6 transition-all hover:shadow-md hover:shadow-emerald-950/5 group flex flex-col justify-between gap-3 shadow-xs"
                    >
                      <div>
                        {/* Topo do Card */}
                        ...
                        <h3
                          ...
                          className="text-base font-bold text-slate-900 group-hover:text-emerald-700 transition-colors cursor-pointer leading-snug line-clamp-2 mt-2"
                        >
                          {idea.title}
                        </h3>
                        ...
                        {/* Dor na Operação */}
                        <div className="mt-3.5 bg-rose-50/50 p-4 rounded-2xl border border-rose-100/80 flex-1 flex flex-col justify-start">
                      ...
                      {/* Rodapé com Comentários, Curtidas e Detalhes */}
                      <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2 mt-auto">
                        ...
                        {idea.status === 'voting' ? (
                          user?.role === 'admin' ? (
                            <div
                              className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1.5 shadow-2xs text-xs text-slate-600 gap-2 select-none"
                            ...
                            <div
                              className="flex items-center bg-amber-50/70 border border-amber-200/80 rounded-xl px-2.5 py-1.5 shadow-2xs text-xs text-amber-900 gap-2 select-none"
                            ...
                            <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl p-0.5 shadow-2xs gap-0.5">
                              {/* Botão Joinha Positivo */}
                              <button
                                type="button"
                                onClick={() => handleVote(idea.id, 1)}
                                className={`px-2 py-1 rounded-lg transition-all flex items-center gap-1 text-xs font-bold cursor-pointer group active:scale-95 ${
                                  userVote === 1
                                    ? 'bg-emerald-600 text-white shadow-xs'
                              ...
                              <span
                                className={`px-1 text-xs font-extrabold select-none ${
                                  score > 0
                                    ? 'text-emerald-700'
                                    : score < 0
                                    ? 'text-rose-600'
                          ...
                          <div className="text-xs text-rose-700 font-bold px-2.5 py-1 bg-rose-50 rounded-xl border border-rose-200 flex items-center gap-1">
                            <AlertCircle className="w-3.5 h-3.5 text-rose-600" />
                            <span>Recusada</span>
                          </div>
                        ) : (
                          <div
                            className="text-xs text-slate-600 font-semibold px-2.5 py-1 bg-slate-50 rounded-xl border border-slate-200 flex items-center gap-1.5 select-none"
            ...
            {/* Visualização em Lista (EXATAMENTE conforme imagem lista.png) */}
            {viewMode === 'list' && (
              <div className="flex flex-col gap-4 animate-fade-in">
                {filteredIdeas.map((idea) => {
                  ...
                  return (
                    <div
                      key={idea.id}
                      className="bg-white border border-slate-200/80 hover:border-emerald-300 rounded-3xl p-5 sm:p-6 transition-all hover:shadow-md hover:shadow-emerald-950/5 group flex flex-col justify-between gap-4 shadow-xs"
                    >
                      <div>
                        {/* Topo do Card (conforme lista.png) */}
                        <div className="flex flex-wrap items-center justify-between gap-2 mb-1.5">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="text-xs font-bold px-3 py-1 rounded-full border bg-emerald-50 text-emerald-800 border-emerald-200">
                              {idea.product}
                        ...
                        {/* Loja, Autor e Data (conforme lista.png: 🏪 Loja Matriz - Supermercado Modelo • Enviado por Wtyhere Monteiro • 15/09/2026) */}
                        ...
                        {/* Box Dor na Operação (conforme lista.png) */}
                      ...
                      {/* Rodapé com Comentários, Votos e Ver Detalhes (conforme lista.png) */}
                      <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <button
                            type="button"
                            onClick={() => setCommentsDrawerIdeaId(idea.id)}
                            className="px-3.5 py-2 rounded-xl text-xs font-bold text-slate-700 hover:text-emerald-800 bg-slate-50 hover:bg-emerald-50/70 border border-slate-200 transition-all flex items-center gap-2 shadow-2xs cursor-pointer"
                            title="Ver comentários no painel lateral"
                          >
                            <MessageSquare className="w-4 h-4 text-emerald-600" />
                        ...
                        {/* Botão Ver Detalhes (conforme lista.png) */}
                        <button
                          type="button"
                          onClick={() => setSelectedIdeaDetailsId(idea.id)}
            ...
            <form id="new-idea-form" onSubmit={handleCreateIdea} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Conciliação Automática de Vendas com Carteiras Digitais no Varejofacil"
        ...
        {activeTab === 'prioritization' && (
          <div className="space-y-6">
            <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-xs">
              ...
              <p className="text-xs sm:text-sm text-slate-500 mt-1">
                Ao final da votação, a equipe de produto Casa Magalhães seleciona as demandas campeãs para entrar na esteira de desenvolvimento do <strong>Varejofacil</strong>.
              </p>
            </div>
      ...
      <footer className="bg-white border-t border-slate-200/80 py-6 text-center text-xs text-slate-400">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <div className="text-[11px] text-slate-400">
            Conectando a operação de supermercados ao P&D de Varejofacil
          </div>
        </div>
      </footer>
